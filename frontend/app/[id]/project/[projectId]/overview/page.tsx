...
export default function ProjectOverview() {
  ...
  useEffect(() => {
    ...
    const getProjectTimeline = async () => {
      try {
        ...
        console.error("Error fetching project timeline:", e.response?.data || e.message);
      }
    };
    getProjectTimeline();
  ...
  // Handle successful invitation - refresh project members
  const onInvite = async () => {
    ...
    try {
      ...
      const errorMessage = e.response?.data?.message || e.message || "Error refreshing project details";
      toast.error(errorMessage);
    }
  };
                  ...
                  <div key={member.id} className="flex items-center gap-3 px-4 py-3 bg-white rounded-md border border-gray-200 hover:border-gray-300 transition-colors">
                    <Avatar className="w-9 h-9">
                      ...
                      <AvatarFallback className="bg-cyan-500 text-white text-sm font-medium">
                        {member.emailuser?.slice(0, 2).toUpperCase() || 'U'}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                                ...
                                <Avatar className="w-8 h-8">
                                  {item.profile ? (
                                    ...
                                    <AvatarFallback className="bg-purple-500 text-white text-xs font-medium">
                                      {item.email?.slice(0, 2).toUpperCase() || 'U'}
                                    </AvatarFallback>
                                  )}
                                </Avatar>
                              </div>
