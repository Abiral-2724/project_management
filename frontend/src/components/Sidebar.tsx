'use client'
import { ChevronRight, Mail, Menu, Plus, UserRound } from 'lucide-react'
...
const Sidebar = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  // unified state for active item (either "home" or projectId)
  ...
  const [userMemberProject, setUserMemberProject] = useState<any[]>([])
  const [userMemberProfileLinks, setUserMemberProfileLinks] = useState<string[]>([])
  ...
  const handleHomeClick = () => {
    setActiveMenu("home")
    router.push(`/${id}/dashboard/home`)
  }

  const handleProjectClick = (projectId: string) => {
    setActiveMenu(projectId)
    router.push(`/${id}/dashboard/project/${projectId}/Overview`)
  }
  ...
  const handleInvite = async (email: string, projectId: string, role: string) => {
    try {
      ...
      arrEmail.push(email)
      await axios.post(`http://localhost:4000/api/v1/${id}/project/${projectId}/addMember/sendInvite`,
        {
        ...
        <nav className="flex-1 p-2 space-y-0.5 min-w-[256px]">
          ...
          <div className="pt-4 pb-2">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => router.push(`http://localhost:3001/${id}/create-project`)} className='cursor-pointer'>
              <Plus className="h-4 w-4 text-gray-400" />
              </button>
            ...
            </div>
            {
              userOwnerProject.map((ownerproject, index) => (
                <NavItem
                  key={index}
                  icon="●"
                  label={ownerproject.projectName}
                  color="text-teal-400"
                  active={activeMenu === ownerproject.id}
                  onClick={() => handleProjectClick(ownerproject.id)}
                />
              ))
            }
          </div>
            ...
            </div>
            {
              userMemberProject.map((memberproject, index) => (
                <NavItem
                  key={index}
                  icon="●"
                  label={memberproject.projectName}
                  color="text-teal-400"
                  active={activeMenu === memberproject.id}
                  onClick={() => handleProjectClick(memberproject.id)}
                />
              ))
            }
          </div>
...
function NavItem({ icon, label, active = false, color = 'text-white', onClick }: { icon: string, label: string, active?: boolean, color?: string, onClick?: () => void }) {
  return (
    <div
